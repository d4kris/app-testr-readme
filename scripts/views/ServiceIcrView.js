define([
  ...
  'modal'
], function ( $, _, Backbone, IcrTemplate, ServiceIcrModel ) {

  var ServiceIcrView = Backbone.View.extend({
    ...
    onChange : function ( event ) {
      ...
      console.log('ServiceIcrView change, input: ' + inputField.value);
      this.sendModel.set(change);
      // enable send button if model passes validation
      check = this.sendModel.validateAll();
      console.log('ServiceIcrView b4send check.isValid :' + check.isValid);
      if (check.isValid) {
    ...
    send : function () {
      ...
      this.sendModel.save(
        ...
        {
          success : function () {
            console.log('ServiceIcrView send-Ticket has been logged');
            self.showAlert('Success!!', 'Ticket has been logged',
                'alert-success');
          },
          error : function () {
            console.log('ServiceIcrView send-error occurred while trying to log the ticket');
            self.showAlert(
                    'Error',

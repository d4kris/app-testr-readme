define([ 'jquery', 'jquery-ui', 'underscore', 'backbone', 'typeahead',
		'text!templates/typeaheadTemplate.html' ], function($, $ui, _,
		Backbone, typeahead, template) {
	/**
	...
	var TypeaheadView = Backbone.View.extend({
		...
		events : {
			...
			'autocompleteclose' : 'setClosevalue'
		},
		...
		initialize : function(options) {
			...
			this.collection.fetch({
				success : function(data) {
					...
					self.flatModel = _.map(self.collection.models, function(
							item) {
				...
				error : function(msg) {
					console
							.log('failed to fetch typeahead data'
									+ self.inputId);
				}
			});
			this.render();
		...
		initTypeahead : function() {
			...
			this.$typeaheadInput.autocomplete({
				minLength : 0,
				...
				return $("<li>").data("item.autocomplete", item.value).append(
						"<a>" + item.value + "</a>").appendTo(ul);
			};
			this.$typeaheadInput.keyup(function(event) {
				event.target.value = event.target.value.toUpperCase();
			});
		},
		setClosevalue : function(event) {
			// user has clicked outside the drop button
			this.open = false;
			console.log('setClosevalue this.open after  -' + this.open);
		},
		showAll : function() {
			console.log('showAll this.open-' + this.open);
			this.$typeaheadInput.focus();
			if (this.open) {
				this.$typeaheadInput.autocomplete('close');
				this.open = false;
			} else {
				this.$typeaheadInput.autocomplete("search", "");
				this.$typeaheadInput.focus();
				this.open = true;
			}
		},
